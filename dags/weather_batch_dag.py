...
# Utilisation d'opérateurs plus basiques en cas d'absence du provider Spark
from airflow.operators.bash import BashOperator
from airflow.utils.dates import days_ago

# Import conditionnel pour le SparkSubmitOperator
try:
    from airflow.providers.apache.spark.operators.spark_submit import SparkSubmitOperator
    SPARK_AVAILABLE = True
except ImportError:
    SPARK_AVAILABLE = False
    # Définir une classe simulée pour éviter les erreurs
    class SparkSubmitOperator(BashOperator):
        def __init__(self, application="", **kwargs):
            bash_command = f"echo '[SIMULATION] Exécution de {application} avec Spark'"
